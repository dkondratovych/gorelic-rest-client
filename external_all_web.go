...
const (
	externalAllWeblApiPath string = "/v2/applications/%d/metrics/data.%s"
)

type ExternalAllWeb struct {
	NewRelicRequest
}

type ExternalAllWebData struct {
	MetricData struct {
...
func (a *ExternalAllWebData) CallCount() int {
	return a.MetricData.Metrics[0].Timeslices[0].Values.CallCount
}

func (a *ExternalAllWebData) AverageResponseTime() float32 {
	return a.MetricData.Metrics[0].Timeslices[0].Values.AverageResponseTime
}

type ExternalAllWebConfig struct {
	Params Params
}

func (ac ExternalAllWebConfig) Init() (*RequestConfig, error) {
	...
	body := url.Values{}
	body.Add("names[]", "External/allWeb")
